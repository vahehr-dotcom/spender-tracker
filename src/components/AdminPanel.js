...
export default function AdminPanel() {
  ...
  const loadUsers = async () => {
    ...
    try {
      const { data: profiles, error: profileError } = await supabase
        .from('user_profiles')
        .select('id, email, first_name, last_name, is_pro, role, gender, created_at, is_banned')
  ...
  const getTierBadge = (user) => {
    ...
    switch (tier) {
      ...
      case 'guest': return { bg: 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)', label: `🎟️ Guest (${user.guest_granted_tier?.toUpperCase() || 'PRO'})` }
